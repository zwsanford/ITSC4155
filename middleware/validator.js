...
export const validateListing = [
    ...
    body('seller', 'Content must be at least 10 characters').isLength({ min: 10 }).trim().escape(),
    ...
    body('image', 'Image URL must not be empty').notEmpty().trim(),
    body('totalOffers', 'Total offers must be valid').trim().escape(),
    body('active', 'Active status must be valid').trim().escape(),
    body('highestOffer', 'Highest offer must be numeric and greater than or equal to 0').trim().escape().isNumeric({ min: 0 }),
];
